...
import org.hlmj.core.listeners.BlockChainListener;
import org.hlmj.store.FlatDB;
import org.hlmj.store.HashStore;
import org.hlmj.store.MasternodeDB;
 ...
 * hlmj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating hlmj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use hlmj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a hlmj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using hlmj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing hlmj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.hlmj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.hlmj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.hlmj.core.NetworkParameters} specified when this context was (auto) created. The
